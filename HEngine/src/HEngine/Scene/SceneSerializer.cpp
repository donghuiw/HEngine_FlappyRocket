...
namespace YAML {
	...
	template<>
	struct convert<HEngine::UUID>
	{
		static Node encode(const HEngine::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, HEngine::UUID& uuid)
		{
					...
					if (scriptFields)
					{
						...
						if (entityClass)
						{
							...
							for (auto scriptField : scriptFields)
							{
								...
								HE_CORE_ASSERT(fields.find(name) != fields.end());
