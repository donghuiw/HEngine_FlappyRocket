...
namespace HEngine
{
	...
	void SceneHierarchyPanel::DrawComponents(Entity entity)
	{
		...
		DrawComponent<SpriteRendererComponent>("Sprite Renderer", entity, [](auto& component)
		{
			...
			if (ImGui::BeginDragDropTarget())
			{
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
				{
					...
					std::filesystem::path texturePath(path); //用于计算资源文件的完整路径
		...
		DrawComponent<Rigidbody2DComponent>("Rigidbody 2D", entity, [](auto& component) 
		{
			...
			if (ImGui::BeginCombo("Body Type", currentBodyTypeString)) { // 第二个参数传入当前选择的文本
				for (int i = 0; i < 3; i++) { // 迭代所有三个类型
					bool isSelected = currentBodyTypeString == bodyTypeStrings[i];
					if (ImGui::Selectable(bodyTypeStrings[i], isSelected)) { // 第二个参数是当前项是否被选中
